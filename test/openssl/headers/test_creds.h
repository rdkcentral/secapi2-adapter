...
#define TESTKC_CONDITIONAL TestCreds::supports(CAPABILITY_LOAD_SYM_SOC_KC) ? TESTKC_SOC : TESTKC_RAW
...
class ProvKey { // NOLINT
public:
    ProvKey() = default; // NOLINT

    ProvKey(std::vector<SEC_BYTE> _key, Sec_KeyContainer _kc)
        : key(std::move(_key)), kc(_kc) {
    }
...
public:
    ...
    static bool supports(Capability cap);
    static void init();
    static void shutdown();

    static std::vector<ProvKey> getWrappedContentKeyChainRsaAes(TestKey contentKey, TestKc kc, SEC_OBJECTID id,
            Sec_KeyType rsaType, Sec_CipherAlgorithm asymAlg);
    static std::vector<ProvKey> getWrappedContentKeyChainRsaAesAes(TestKey contentKey, TestKc kc, SEC_OBJECTID id,
            Sec_KeyType rsaType, Sec_CipherAlgorithm asymAlg, Sec_KeyType aesType, Sec_CipherAlgorithm symAlg);
    static std::vector<ProvKey> getWrappedContentKeyChainRsaAesRsaAesAes(TestKey contentKey, TestKc kc, SEC_OBJECTID id,
            Sec_KeyType rsaType, Sec_CipherAlgorithm asymAlg, Sec_KeyType aesType, Sec_CipherAlgorithm symAlg,
            Sec_CipherAlgorithm ckSymAlg);
    static std::vector<ProvKey> getWrappedContentKeyChainEcAes(TestKey contentKey, TestKc kc, SEC_OBJECTID id,
            Sec_CipherAlgorithm asymAlg);
    static std::vector<ProvKey> getWrappedContentKeyChainEcAesAes(TestKey contentKey, TestKc kc, SEC_OBJECTID base_id,
            Sec_KeyType aesType, Sec_CipherAlgorithm asymAlg, Sec_CipherAlgorithm symAlg);

    static ProvKey* wrapAesWithAes(const SEC_BYTE* clear, Sec_KeyType type, const SEC_BYTE* wrapping,
            Sec_KeyType wrappingType, SEC_OBJECTID wrappingId, Sec_CipherAlgorithm symAlg);
    static ProvKey* wrapAesWithEc(const SEC_BYTE* clear, Sec_KeyType type, EC_KEY* ec_key, SEC_OBJECTID wrappingId,
            Sec_CipherAlgorithm asymAlg);
    static ProvKey* wrapAesWithRsa(const SEC_BYTE* data, Sec_KeyType type, RSA* rsa_key, SEC_OBJECTID wrappingId,
            Sec_CipherAlgorithm asymAlg);
    static ProvKey* wrapRsaWithAes(RSA* rsa, const SEC_BYTE* wrapping, Sec_KeyType wrappingType,
            SEC_OBJECTID wrappingId, Sec_CipherAlgorithm symAlg);
    static std::string b64_encode(const void* in, size_t in_length);
};
