...
static Sec_Result DH_generate_key(DH* dh, SEC_BYTE* publicKey, SEC_SIZE pubKeySize, SEC_SIZE* out_len) {
...
#if OPENSSL_VERSION_NUMBER < 0x10100000L
    if (pubKeySize < static_cast<SEC_SIZE>(BN_num_bytes(dh->pub_key))) {
        SEC_LOG_ERROR("Buffer to small");
        return SEC_RESULT_FAILURE;
    }

    *out_len = BN_bn2bin(dh->pub_key, publicKey);
    if (*out_len < pubKeySize) {
        memmove(publicKey + pubKeySize - *out_len, publicKey, *out_len);
        memset(publicKey, 0, pubKeySize - *out_len);
    }
#else
    ...
    *out_len = BN_bn2bin(pub_key, publicKey);
#endif
...
static Sec_Result DH_compute(DH* dh, SEC_BYTE* pub_key, SEC_SIZE pub_key_len, SEC_BYTE* key, SEC_SIZE key_len,
        SEC_SIZE* written) {
    if (key_len < static_cast<SEC_SIZE>(DH_size(dh))) {
