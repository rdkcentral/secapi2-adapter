...
public:
    ...
    Sec_KeyHandle* provisionKey(SEC_OBJECTID id, Sec_StorageLoc location, TestKey key, TestKc,
            bool softWrap = SEC_FALSE);
    Sec_KeyHandle* provisionKey(SEC_OBJECTID id, Sec_StorageLoc location, const SEC_BYTE* data, SEC_SIZE len,
            Sec_KeyContainer kc, bool softWrap = SEC_FALSE);
    ...
    Sec_SignatureHandle* signatureHandleacquireSignature(Sec_SignatureAlgorithm algorithm, Sec_SignatureMode mode,
            Sec_KeyHandle* keyHandle); void releaseSignature(Sec_SignatureHandle* signatureHandlesig);
